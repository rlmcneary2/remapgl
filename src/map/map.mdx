...
A Map component can accept a number of other components as children. Data at a specific point is displayed on a map using a [Marker](/src/marker/marker.mdx). Information that applies to paths or regions on a map can be applied by [Layer](/src/layer/mapboxgl/layer.mdx) components.
